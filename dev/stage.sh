...
# command line containing this path, so it has to be somewhere nobody else can
...
if [ -n "${CLIAMP_DEV_STAGE:-}" ]; then
  STAGE="$CLIAMP_DEV_STAGE"
elif [ -n "${XDG_RUNTIME_DIR:-}" ]; then
  STAGE="$XDG_RUNTIME_DIR/omarchy-cliamp-dev"
else
  echo "dev: no XDG_RUNTIME_DIR - set CLIAMP_DEV_STAGE to a directory only you can write" >&2
  exit 1
fi
